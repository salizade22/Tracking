    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            for pos in self.legalPositions:
                beliefs[pos] = 0
            prisonCell = self.getJailPosition()
            beliefs[prisonCell] = 1.0
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                res[newPos] += prob * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        perCapita = (self.numParticles / (len(self.legalPositions)))

        for i in range(int(perCapita)):
            for pos in self.legalPositions:
    ...
    def observe(self, observation, gameState):
        ...
        for i in range(n):
            trueDistance = util.manhattanDistance(pacmanPosition, self.particles[i])
            weights[i] *= emissionModel[trueDistance]
        ...
        if all([w == 0 for w in weights]):
            ...
            tmp = util.Counter()
            for i in range(n):
                tmp[self.particles[i]] += weights[i]

            particles = []
            for i in range(n):
                particle = util.sample(tmp)
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            particle = util.sample(newPosDist)
            particles.append(particle)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numGhosts):
            allghostpositions.append(self.legalPositions)
        permutations = list(itertools.product(*allghostpositions))
        ...
        for i in range(n):
            ...
            for j in range(self.numGhosts):
                if noisyDistances[j] is not None:
                    trueDistance = util.manhattanDistance(pacmanPosition, self.particles[i][j])
                    weight *= emissionModels[j][trueDistance]

            tmp[self.particles[i]] += weight
